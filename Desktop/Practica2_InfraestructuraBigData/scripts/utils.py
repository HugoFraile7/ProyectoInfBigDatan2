...
def extract_sql_to_dataframes(sql_file_path_minio, mysql_config):
    """
    ...
    """
    print(f"📥 Descargando archivo SQL desde MinIO: {sql_file_path_minio}...")
    local_sql_path = '/tmp/dump.sql'
    bucket, key = sql_file_path_minio.split('/', 1)
    download_file_from_minio(bucket, key, local_sql_path)

    print(f"📑 Ejecutando archivo SQL: {local_sql_path} en MariaDB...")
...
def extract_json_to_dataframe(json_file_path_minio):
    """
    Descarga un archivo JSON desde MinIO y lo convierte en DataFrame.
    """
    print(f"📥 Descargando JSON desde MinIO: {json_file_path_minio}...")
    local_json_path = '/tmp/avisamadrid.json'
    bucket, key = json_file_path_minio.split('/', 1)
    download_file_from_minio(bucket, key, local_json_path)

    print(f"📑 Leyendo archivo JSON desde: {local_json_path}")
    try:
        df = pd.read_json(local_json_path)
        print(f"✅ JSON cargado con {len(df)} registros y {len(df.columns)} columnas")
    except ValueError as e:
        raise Exception(f"❌ Error leyendo el archivo JSON: {e}")
